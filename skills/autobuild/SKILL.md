...
- **`trackerKind: 'pinax'`** — a repo tracked with [pinax](https://github.com/antikas/pinax): read the next item with `pinax next` (critical-path order over the typed graph); on **accept** append a completion event (`pinax done <id>`) and commit it (explicit path); on **park** append a block/park event with the structured reason. The `.ergon` event log is then the SSOT for what the run did, replayable on any machine.
...
**Claude-side quota instrumentation.** `quota.py` (this folder) reads live subscription utilisation from the client's own **undocumented** `/api/oauth/usage` endpoint. Preflight (multi-lane runs) probes it, cools any CRITICAL model-scoped limit with its real reset time, and applies the weekly ceiling. Non-fatal and config-gated (`claudeQuotaProbe:false` disables); undocumented endpoint, so it is a *soft* signal (probe failure → proceed). Run it yourself anytime: `python quota.py` (`--json` / `--gate weekly_all:85`).
